...
##### create product_revenue_query with the query from parts 3 and 4 which get the total revenue from each product category
    product_revenue_query ="""SELECT SUM(unit_price), product_category FROM invoices 
...
#### write a function which loops through the product_revenue variable and creates two lists - one with product categories and one with total revenue values
    def generate_lists (input):
        ...
        product_list=[]
        for x in input:
            revenue_list.append(x[0])
            product_list.append(x[1])
        return (revenue_list, product_list)  #returns a tuple of lists  
    input = generate_lists(product_revenue) #lists are returned as a tuple of arrays.     
